...
if (!imageData.getHierarchy().getAnnotationObjects().contains(annotation)) {
    println("Error al crear la anotación.")
    return
} else {
    println("Anotación creada con éxito.")
}
...
// Filtrar detecciones por tamaño mínimo y máximo
double minSizeMicrons = 0.01
double maxSizeMicrons = 0.18

def filteredDetections = detections.findAll { detection ->
    def measurementList = detection.getMeasurementList()
    def area = measurementList.getMeasurementValue("Area µm^2")
    return area >= minSizeMicrons && area <= maxSizeMicrons
}

println("Number of detections after filtering: " + filteredDetections.size())

// Eliminar detecciones que no cumplen con los criterios de tamaño
