...
project.getImageList().each { entry ->
    ...
    def exportDetections = imageData.getHierarchy().getDetectionObjects().findAll {
        it.getPathClass()?.toString() == "Melanosome"
    }

    if (!exportDetections.isEmpty()) {
        def imageName = entry.getImageName()
        def csvFile = buildFilePath(outputDir, imageName + "_Melanosome.csv")
        ...
        println("ℹ️ No se encontraron objetos 'Melanosome' en: " + entry.getImageName())
    }
}

// Guardar combinado
def combinedFile = buildFilePath(outputDir, "melanosome_combined.csv")
